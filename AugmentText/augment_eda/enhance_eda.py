...
random.seed(2019)
key_word_list = ["BUPT-mobile", "BT", "BUPT-protal"]
...
word2vec_model = load_word2vec_model(
    word2_vec_path, limit_words=10000, binary_type=False, encoding_type='utf-8')
...
def synonym_replacement(words, n, key_words):
    ...
    for random_word in random_word_list:
        ...
        if len(sim_synonyms) >= 1 and random_word not in key_words and not is_total_english(
...
def eda(sentence, alpha_sr=0.1, alpha_ri=0.1, alpha_rs=0.1, p_rd=0.1, num_aug=9, key_words=[]):
    ...
    if num_aug < len(augmented_sentences):
